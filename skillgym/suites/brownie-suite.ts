...
const brownieSuite = [
    ...
    assert(report) {
      assert.fileReads.includes(report, /skills\/brownie\/references\/getting-started\.md$/);
  ...
  },
  {
    ...
    assert(report) {
      assert.fileReads.includes(report, /skills\/brownie\/references\/android-usage\.md$/);
