...
Installed third-party packages like `react-native-screens` or others may rely on transitive dependencies in their `build.gradle` file. For bare React Native apps, `brownfield-gradle-plugin` auto resolves these by default via the (experimental) `experimentalIncludeTransitiveDependencies` option — see [Configure Maven Publishing](/docs/getting-started/android#6-configure-maven-publishing) for details.

For Expo apps, this works out of the box unconditionally.

Upgrading an existing app requires no changes on your side — in particular, you do not need to remove the hand-written `tasks.register("removeDependenciesFromModuleFile")` block if you have one, since the plugin's own task is named differently and the two do not conflict. Be aware of two behaviour changes once this is enabled: dependencies your embedded modules need will start appearing in the published POM, and a version you pinned yourself may be **raised** (never lowered) if an embedded module requires a higher one.

If you've set `experimentalIncludeTransitiveDependencies = false` (e.g. to opt out while this is still experimental), use one of the options below instead.
...
## [Android] Defining Missing Dimension Strategies

This maybe required if any third party library has defined product flavors and dimensions. If so, then the consuming modules, in brownfield case, `:app` and `:brownfieldlib` need 
to define the `missingDimensionStrategy`.

:::info
This should be only required for bare react native. For Expo, this is already handled by `@callstack/react-native-brownfield` expo config plugin.
You only need to define `android.expo.missingDimensionStrategies` using the brownfield config.
:::
...
android {
    defaultConfig {
        ...
        versionName "1.0"
        missingDimensionStrategy("type", "alpha")
    }
}
```
