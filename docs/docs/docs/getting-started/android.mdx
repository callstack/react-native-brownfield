...
> ```
>
> Do **not** combine the hand-written `tasks.register("removeDependenciesFromModuleFile")` block above with `experimentalIncludeTransitiveDependencies = true` (the default) in the same module — the plugin already performs that same removal (plus dependency injection) for every publication whenever the option is enabled, so keeping both means two tasks race to rewrite the same generated `module.json`, corrupting whichever one runs last.
