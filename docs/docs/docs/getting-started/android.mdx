...
> ```
>
> Do **not** combine this option with the hand-written `tasks.register("removeDependenciesFromModuleFile")` block above in the same module — the plugin already performs that same removal (plus dependency injection) for every publication once the option is enabled, so keeping both means two tasks race to rewrite the same generated `module.json`, corrupting whichever one runs last.
