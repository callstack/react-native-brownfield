...
`OnJSBundleLoaded` receives `JSBundleTimings` on the main thread. Its nullable
`jsBundleLoadTime`, `jsBundleEvaluationTime`, and sorted `timeline` fields describe
completed startup intervals. This replaces the Boolean callback argument used by
earlier releases.
