...
struct IosApp: App {
    ...
    init() {
        ...
        ReactNativeBrownfield.shared.startReactNative { timings in
            print("React Native has been loaded jsBundleLoadTime=\(String(describing: timings.jsBundleLoadTime)) jsBundleEvaluationTime=\(String(describing: timings.jsBundleEvaluationTime))")
        }
    }
...
- `targetSdkVersion` (`number`, default: inherited from the generated Expo app project when available)
  - Target Android SDK version used to build the library.
- `compileSdkVersion` (`number`, default: inherited from the generated Expo app project when available)
