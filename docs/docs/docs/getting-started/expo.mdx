...
- `targetSdkVersion` (`number`, default: inherited from the generated Expo app project when available). Applied to the library's `testOptions` and `lint`, since Android libraries have no target SDK of their own
