...
| `onBundleLoaded` | No       | `void(^)(JSBundleTimings *)` | Callback invoked on the main thread after JS bundle is fully loaded. Receives `JSBundleTimings`. |
...
```objc
[[ReactNativeBrownfield shared] startReactNative:^(JSBundleTimings *timings){
    NSLog(@"React Native started loadMs=%@ executeMs=%@", timings.loadMs, timings.executeMs);
}];
```
...
[[ReactNativeBrownfield shared] startReactNativeWithLaunchOptions:launchOptions
                                                    preloadBundle:YES
                                                   onBundleLoaded:^(JSBundleTimings *timings){
    NSLog(@"React Native bundle loaded loadMs=%@ executeMs=%@", timings.loadMs, timings.executeMs);
}];
```
