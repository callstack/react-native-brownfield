...
ReactNativeBrownfield.initialize(this, packages) {
  timings -> println(timings.jsBundleEvaluationTime)
}
```
...
| waitForFullDisplay | No | `Boolean` | Wait for JavaScript to mark the injected `brownfieldPresentationID`. Default: `false`. |
| collectThreadMetrics | No | `Boolean` | Collect JS/UI callback cadence and busy estimates. Default: `false`. |
...
```kotlin
val measured = ReactNativeBrownfield.shared.createView(
  activity,
  "ReactNative",
  launchOptions = props,
  waitForFullDisplay = true,
  collectThreadMetrics = true
) { metrics -> println(metrics.fullDisplay?.duration) }
```

`OnJSBundleLoaded` now receives `JSBundleTimings` instead of the former Boolean.
Update callbacks that read `initialized` to read the nullable load/evaluation
durations. See `PERFORMANCE.md` for metric boundaries and availability.
...
ReactNativeBrownfield.shared.postMessage(payload.toString())
```
...
val listener = OnMessageListener { message ->
...
| onMetrics | No | `OnDisplayMetrics` | Completion callback; assign `fragment.onMetrics` again after restoration. |
...
ReactNativeFragment.createReactNativeFragment(
  "ReactNative", bundle, waitForFullDisplay = true,
  collectThreadMetrics = true
) { metrics -> println(metrics.fullDisplay?.duration) }
```
...
- _(optional)_ performance flags under `ARG_WAIT_FOR_FULL_DISPLAY` and `ARG_COLLECT_THREAD_METRICS`; callback properties are not serialized
