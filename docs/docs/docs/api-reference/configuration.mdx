...
| `android.expo.targetSdkVersion`     | `number`  | Target Android SDK version for the library. Used on Expo SDK versions below 58; ignored on 58 and later.                                 |
| `android.expo.compileSdkVersion`    | `number`  | Compile Android SDK version used to build the library. Defaults to 37 on Expo SDK 58 and later, otherwise 35.                            |
...
| `android.expo.missingDimensionStrategies` | `string[]` | Gradle `missingDimensionStrategy` arguments: dimension name followed by one or more flavors.                                            |
...
| `android.expo.useLocalMaven`        | `boolean` | Load the Brownfield Gradle plugin published to local maven instead of using from remote. Disabled by default.                                                         |
