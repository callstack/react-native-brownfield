...
- `bundleAssetPath`: `String` - Path to react native bundle asset file (when loading from app assets), appended to `asset://`; if this path starts with `file://`, the JS bundle will be loaded from a local file, not from app's assets.
