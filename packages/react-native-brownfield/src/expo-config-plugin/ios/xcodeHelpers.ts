...
export function addFrameworkTarget(
  ...
  if (!frameworkTarget) {
    throw new Error(`Failed to create framework target: ${frameworkName}`);
  }
  ...
  log(`Successfully added framework target: ${frameworkName}`);
}
...
function copyBundleReactNativePhase(
  ...
  if (!shellScriptPhases) {
    log(`No shell script build phases found, skipping ${buildPhaseName}`);
    return;
  }
  ...
  if (!existingPhaseUuid) {
    log(`Could not find "${buildPhaseName}" build phase, skipping`);
    return;
  }
  ...
  if (nativeTargets && nativeTargets[targetUuid]) {
    ...
    if (target.buildPhases) {
      ...
      const alreadyExists = target.buildPhases.some(
...
/**
 * Creates the Swift interface file content for the framework
 */
export function getFrameworkInterfaceContent(frameworkName: string): string {
  return `//
//  ${frameworkName}.swift
//  ${frameworkName}
//
